...
import { getFacilityTypes, getRentTypes, createFacility } from '../../service/facilityService';
...
const CreateFacility = () => {
    ...
    useEffect(() => {
        const fetchTypes = async () => {
            try {
                const [fTypes, rTypes] = await Promise.all([
                    getFacilityTypes(),
                    getRentTypes()
                ]);
                ...
                toast.error("Không thể tải danh mục loại dịch vụ");
            }
        };
        fetchTypes();
    }, []);
    ...
    const handleSubmit = async (values) => {
        try {
            ...
            if (facilityTypeId === '1') { // Villa
                delete dataToSubmit.freeService;
            } else if (facilityTypeId === '2') { // House
                delete dataToSubmit.poolArea;
                delete dataToSubmit.freeService;
            } else { // Room
                dataToSubmit.freeService = values.freeService.join(", ");
            ...
            await createFacility(dataToSubmit);
                ...
                <Col md={8}>
                    <Card className="shadow-lg border-0 rounded-4">
                        <Card.Header className="bg-premium-gradient text-white text-center py-4 border-0">
                            <h2 className="mb-0 fw-bold">Thêm Mới Dịch Vụ</h2>
                                    ...
                                    <Form>
                                        <Row>
                                            <Col md={12} className="mb-4">
                                                ...
                                                <Field name="facilityTypeId" as="select" className="form-select form-control-lg border-2 shadow-none shadow-sm">
                                                    ...
                                                    <div className="d-flex flex-wrap gap-4 mt-2">
                                                        {FREE_SERVICES.map((service, index) => (
                                                            <BsForm.Check
                                                                ...
                                                                label={service}
                                                                value={service}
                                                                checked={values.freeService.includes(service)}
                                                                onChange={(e) => {
                                                                    const { checked, value } = e.target;
                                                                    const nextValue = checked
                                                                        ? [...values.freeService, value]
                                                                        : values.freeService.filter(v => v !== value);
